...
public class KakaoLinkServiceTest extends KakaoTestCase {
    ...
    public void setup() {
        ...
        NetworkService networkService = spy(new TestNetworkService(new Handler(ShadowLooper.getMainLooper()), new RoboExecutorService()));
        this.service = spy(new KakaoLinkService(linkService, imageService, networkService));
    ...
    public void sendWithLowerTalkVersion() {
        ...
        ShadowApplication.runBackgroundTasks();
        ShadowLooper.runUiThreadTasksIncludingDelayedTasks();
        verify(service).openUrlWithCustomTab(eq(activity), any(Uri.class));
    }
    ...
    public void sendWithLongUri() {
        ...
        doThrow(new KakaoException(KakaoException.ErrorType.URI_LENGTH_EXCEEDED, "uri length exceeded"))
                .when(linkService).kakaoLinkIntent(eq(activity), (String) isNull(), any(JSONObject.class));
